...
The correlation analysis revealed a statistically significant positive correlation (r = 0.12, p < 0.001) between dLMC neural power and gesture velocity in one participant, supporting our hypothesis that affective gestures co-occur with more emotionally charged, higher-pitched vocalization. This moderate relationship suggests that the dLMC, besides its role in laryngeal motor control, may also be involved in coordinating upper limb movements to produce affective speech and congruent gestures. However, further discussion on an expanded role of the dLMC must be substantiated by functional co-synchrony and temporal phase-locking with the motor cortex primarily responsible for gesturing. Correlation in one participant does not imply causation, and additional analyses and experiments would be necessary to establish any causal relationship between neural activity and movement. Moreover, the moderate correlation coefficient indicates that other factors may influence the relationship between neural power and gesture velocity.

![image](https://github.com/user-attachments/assets/a0cd2165-182c-4209-9f86-6aeb0e25b00f)
##### Figure 2. Extracted ECoG electrodes of the dLMC (blue) and wrist motor cortex (green). The spectral coherence is calculated based on the neural power time series from these two sets of electrodes.
